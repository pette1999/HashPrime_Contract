...
contract Common is Test, MultiRewardDistributorCommon {
    ...
    function assertInitialMarketState(MultiRewardDistributor distributor, uint256 endTime) internal view {
        MarketConfig memory config = distributor.marketConfigs(address(hToken), 0);

        assertEq(HTokenInterface(hToken).totalSupply(), 0);
        ...
        assertEq(config.supplyGlobalTimestamp, vm.getBlockTimestamp());
        assertEq(config.borrowGlobalIndex, 0);
        assertEq(config.borrowGlobalTimestamp, vm.getBlockTimestamp());
        assertEq(config.supplyEmissionsPerSec, 0.54321e18);
        assertEq(config.borrowEmissionsPerSec, 0.54321e18);
    }

    function createDistributorWithOddValuesAndConfig() internal returns (IMultiRewardDistributor) {
        faucetToken.allocateTo(address(this), 1.2345e18);
        faucetToken.approve(address(hToken), 1.2345e18);

        IMultiRewardDistributor distributor = IMultiRewardDistributor(address(new MultiRewardDistributor()));
        ...
        /// wire proxy up
        distributor = IMultiRewardDistributor(address(proxy));

        uint256 endTime = vm.getBlockTimestamp() + 1000;

        // Add config + send emission tokens
        emissionToken.allocateTo(address(distributor), 100e18);
        distributor._addEmissionConfig(hToken, address(this), address(emissionToken), 0.54321e18, 0.54321e18, endTime);
    ...
    function createDistributorWithRoundValuesAndConfig(
    ...
    ) internal returns (IMultiRewardDistributor distributor) {
        faucetToken.allocateTo(address(this), tokensToMint);
        faucetToken.approve(address(hToken), tokensToMint);

        distributor = IMultiRewardDistributor(address(new MultiRewardDistributor()));
        bytes memory initdata =
            abi.encodeWithSignature("initialize(address,address)", address(comptroller), address(this));
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(address(distributor), proxyAdmin, initdata);
        /// wire proxy up
        ...
        // 1 year of rewards
        uint256 endTime = vm.getBlockTimestamp() + (60 * 60 * 24 * 365);

        // Add config + send emission tokens
        emissionToken.allocateTo(address(distributor), 100e18);
        distributor._addEmissionConfig(
            hToken, address(this), address(emissionToken), supplyEmissionsPerSecond, borrowEmissionsPerSecond, endTime
        );
    }
    ...
    function mint(address user, uint256 amountToSupply) public {
        ...
        vm.prank(user);
        faucetToken.approve(address(hToken), amountToSupply);

        vm.prank(user);
        hToken.mint(amountToSupply);
    }

    function borrow(address user, uint256 amountToBorrow) public {
        vm.prank(user);
        hToken.borrow(amountToBorrow);
    }
}
...
contract MultiRewardBorrowSideDistributorUnitTest is Test, ExponentialNoError, Common {
    ...
    function testBorrowerHappyPath() public {
        ...
        IMultiRewardDistributor distributor = createDistributorWithRoundValuesAndConfig(
        ...
        marketsToEnter[0] = address(hToken);
        comptroller.enterMarkets(marketsToEnter);

        assertEq(HTokenInterface(hToken).totalSupply(), 2e18);
        ...
        vm.warp(vm.getBlockTimestamp() + warpSeconds);
    ...
    function testBorrowEvents() public {
        ...
        IMultiRewardDistributor distributor = createDistributorWithOddValuesAndConfig();

        hToken.mint(1.2345e18);
        hToken.borrow(0.5e18);

        vm.warp(startTime + 5);

        // 5 blocks @ 0.54321 tokens per second over a total of 1.2345 hTokens is
        // the total emissions per hToken, which is the root of the underlying index.
        Double memory delta1 =
            fraction(5 * 0.54321e18, div_(hToken.totalBorrows(), Exp({mantissa: hToken.borrowIndex()})));
        ...
        Double memory delta2 =
            fraction(10 * 0.54321e18, div_(hToken.totalBorrows(), Exp({mantissa: hToken.borrowIndex()})));
...
contract MultiRewardSupplySideDistributorUnitTest is Test, ExponentialNoError, Common {
    ...
    function testSupplyingBeforeRewardsPath() public {
        ...
        faucetToken.approve(address(hToken), 2e18);
        hToken.mint(2e18);

        // THEN create the distributor and add a config
        IMultiRewardDistributor distributor = createDistributorWithRoundValuesAndConfig(2e18, 0.5e18, 0.5e18);

        // Make sure we have an expected amt of hTokens issued
        assertEq(HTokenInterface(hToken).totalSupply(), 2e18);
        ...
        // Claim rewards
        vm.prank(address(comptroller));
        distributor.disburseSupplierRewards(hToken, address(this), true);
    ...
    function testSupplierHappyPath() public {
        ...
        vm.warp(vm.getBlockTimestamp() + 12345);
    ...
    function testMultiRewardEmitterWithOddInputs() public {
        ...
        assertEq(HTokenInterface(hToken).totalSupply(), 1.2345e18);
        ...
        uint256 supplierTokens = HTokenInterface(hToken).balanceOf(address(this));
    ...
    function testMultipleEmissionTokens() public {
        ...
        distributor._addEmissionConfig(
            hToken, address(this), address(emissionToken), EMISSION_TOKEN_1, EMISSION_TOKEN_1, endTime
        );

        faucetToken.approve(address(hToken), EMISSION_TOKEN_1 * 10);
        faucetToken.allocateTo(address(this), EMISSION_TOKEN_1 * 10);
        hToken.mint(EMISSION_TOKEN_1 * 10);
        ...
        // Emit 1 token per second
        distributor._addEmissionConfig(
            hToken, address(this), address(emissionToken2), EMISSION_TOKEN_2, EMISSION_TOKEN_2, endTime
        );
        ...
        assertEq(emissionToken2.balanceOf(address(this)), EMISSION_TOKEN_2 * 10); // 10 secs worth of accrual @ 1 token/sec == 10 tokens
    }
    ...
    function testOutstandingRewardCalcs() public {
        ...
        RewardWithHToken[] memory rewardInfo = distributor.getOutstandingRewardsForUser(address(this));
        ...
        RewardInfo[] memory rewards = distributor.getOutstandingRewardsForUser(hToken, address(this));
    ...
    function testOutstandingRewardCalcsOneSecond() public {
        ...
        vm.warp(vm.getBlockTimestamp() + timeDelta1);

        RewardInfo[] memory rewardInfo = distributor.getOutstandingRewardsForUser(hToken, address(this));
        ...
        vm.warp(vm.getBlockTimestamp() + timeDelta2);

        rewardInfo = distributor.getOutstandingRewardsForUser(hToken, address(this));
    ...
    function testLotsOfEmissionTokens() public {
        ...
        FaucetToken[] memory emissionTokens = new FaucetToken[](ITERATION_COUNT);
        ...
        for (uint256 i = 0; i < ITERATION_COUNT; i++) {
            ...
            distributor._addEmissionConfig(
                hToken, address(this), address(multiEmissionToken), 1e18 * (i + 1), 2e18 * (i + 1), endTime
            );
        ...
        faucetToken.approve(address(hToken), 10e18);
        hToken.mint(10e18);
    ...
    function testDifferentNumOfSuppliers() public {
        ...
        distributor._addEmissionConfig(
            hToken, address(this), address(emissionToken), 1e18, 0, vm.getBlockTimestamp() + 1000
        );
        ...
        for (uint256 i = 0; i < USERS; i++) {
            ...
            vm.prank(user);
            faucetToken.approve(address(hToken), (i + 1) * 1e18);

            vm.prank(user);
            hToken.mint((i + 1) * 1e18);

            users[i] = user;
        }

        uint256 SLEEP_TIME = 123;
        vm.warp(vm.getBlockTimestamp() + SLEEP_TIME);
...
contract MultiRewardDistributorCommonUnitTest is Test, ExponentialNoError, Common {
    ...
    function testDoubleClaiming() public {
        ...
        uint256 endTime = vm.getBlockTimestamp() + 600;

        // Add config + send emission tokens
        emissionToken.allocateTo(address(distributor), 1_000_000e18);
        distributor._addEmissionConfig(hToken, address(this), address(emissionToken), 0.5e18, 0.5e18, endTime);
    ...
    function testRewardsEndingTime() public {
        ...
        vm.warp(vm.getBlockTimestamp() + 99);
        comptroller.claimReward();
        assertEq(emissionToken.balanceOf(address(this)), 100e18);

        vm.warp(vm.getBlockTimestamp() + 499);
        comptroller.claimReward();
    ...
    function testEndTimeValidation() public {
        ...
        distributor._addEmissionConfig(
            ...
            0.5e18,
            0.5e18,
            vm.getBlockTimestamp() - 1 // Make sure this is checked and fails
        );
    }
    ...
    function testRenewingRewards() public {
        ...
        distributor._updateEndTime(hToken, address(emissionToken), vm.getBlockTimestamp() + 100);
        ...
        // Accrue some rewards and claim them
        vm.warp(vm.getBlockTimestamp() + 49);
        comptroller.claimReward();
    ...
    function testVariousSuppliersAndBorrowers(uint256 SUPPLY_SPEED, uint256 BORROW_SPEED)
        public
        returns (User[10] memory)
    {
        ...
        distributor._addEmissionConfig(
            ...
            SUPPLY_SPEED,
            BORROW_SPEED,
            vm.getBlockTimestamp() + (60 * 60 * 24 * 365) // 1 yr
        );
        ...
        vm.warp(vm.getBlockTimestamp() + TIME_DELTA);

        // Tally up total supplied/borrowed
        uint256 totalSupplied = (hToken.totalSupply() * hToken.exchangeRateStored()) / 1e18;
        uint256 totalBorrows = hToken.totalBorrows();
    ...
    function testUpdateSupplyEmissionCaps() public {
        ...
        vm.expectRevert("Cannot set a supply reward speed higher than the emission cap!");
        distributor._updateSupplySpeed(hToken, address(emissionToken), currentEmissionCap + 1);
    }
    ...
    function testUpdateBorrowEmissionCaps() public {
        ...
        vm.expectRevert("Cannot set a borrow reward speed higher than the emission cap!");
        distributor._updateBorrowSpeed(hToken, address(emissionToken), currentEmissionCap + 1);
    }
    ...
    function testBorrowRewardsFuzz(uint256 rewardsPerSecond, uint256 secondsToWarp) public {
        ...
        distributor._addEmissionConfig(
            ...
            0,
            rewardsPerSecond,
            vm.getBlockTimestamp() + (365 days) // 1 yr
        );
        emissionToken.allocateTo(address(distributor), type(uint256).max);
        ...
        vm.warp(vm.getBlockTimestamp() + secondsToWarp);
    ...
    function testBorrowSupplyRewardsFuzz(
        ...
        rewardsPerSecondBorrow = _bound(rewardsPerSecondBorrow, 0.0001e18, 1e18);
        rewardsPerSecondSupply = _bound(rewardsPerSecondSupply, 0.0001e18, 1e18);
        secondsToWarp = _bound(secondsToWarp, 1, 365 days);
    ...
    function testGlobalSupplyIndexProperlyUpdates() public {
        ...
        uint256 currSupplyIndex = distributor.getGlobalSupplyIndex(address(hToken), 0);
    ...
    function testAccurateRewardAmountsSupply() public {
        ...
        uint256 endTime = vm.getBlockTimestamp();
        uint256 expectedRewards = (endTime - startTime) * supplySpeed;
        uint256 currSupplyIndex = distributor.getGlobalSupplyIndex(address(hToken), 0);

        assertTrue(currSupplyIndex > 1e36);

        assertApproxEqRel(rewardsSpent, expectedRewards, 0.00001e18);
        /// 1 basis point of deviation allowed
    }
    ...
    function testSetNewSupplyEmissionsToCurrentFails() public {
        distributor._addEmissionConfig(
            hToken, address(this), address(emissionToken), 1e18, 1e18, vm.getBlockTimestamp() + 365 days
        );

        vm.expectRevert("Can't set new supply emissions to be equal to current!");
        distributor._updateSupplySpeed(hToken, address(emissionToken), 1e18);
    }
    ...
    function testSetNewBorrowEmissionsToCurrentFails() public {
        ...
        distributor._updateBorrowSpeed(hToken, address(emissionToken), 1e18);
    }
    ...
    function testSetNewEndTimeLtOrEqualCurrentEndTimeFails() public {
        ...
        distributor._updateEndTime(hToken, address(emissionToken), vm.getBlockTimestamp() + 365 days);
    }
    ...
    function testSetNewEndTimeCurrentTimestampFails() public {
        ...
        distributor._updateEndTime(hToken, address(emissionToken), vm.getBlockTimestamp());
    }
    ...
    function testSetNewEndTimePastFails() public {
        ...
        uint256 invalidStartTime = vm.getBlockTimestamp() + 380 days;
        vm.warp(vm.getBlockTimestamp() + 400 days);
        /// past end time now,

        vm.expectRevert("_newEndTime MUST be > block.timestamp");
        distributor._updateEndTime(hToken, address(emissionToken), invalidStartTime);
    }

    function testGetConfigForNonExistentMarketFails() public {
        vm.expectRevert("Unable to find emission token in hToken configs");
        distributor.getConfigForMarket(HToken(address(0)), address(emissionToken));
    }
    ...
    function testSetNewSupplyEmissionsAdminSucceeds() public {
        ...
        distributor._updateSupplySpeed(hToken, address(emissionToken), 0.5e18);

        MarketConfig memory config = distributor.getConfigForMarket(hToken, address(emissionToken));

        assertEq(config.owner, address(this));
        assertEq(config.emissionToken, address(emissionToken));
        assertEq(config.endTime, vm.getBlockTimestamp() + 365 days);
        assertEq(config.supplyGlobalIndex, distributor.initialIndexConstant());
    ...
    function testSetNewBorrowEmissionsAdminSucceeds() public {
        ...
        distributor._updateBorrowSpeed(hToken, address(emissionToken), 0.5e18);
    ...
    function testUpdateOwnerConfigOwnerSucceeds() public {
        distributor._addEmissionConfig(
            hToken,
            address(1),
            /// address 1 is owner of this config
            address(emissionToken),
            1e18,
            1e18,
            vm.getBlockTimestamp() + 365 days
        );
        ...
        vm.prank(address(1));
        distributor._updateOwner(hToken, address(emissionToken), address(this));

        config = distributor.getConfigForMarket(hToken, address(emissionToken));
    ...
    function testUpdateOwnerNonConfigOwnerFails() public {
        ...
        vm.prank(address(2));
        vm.expectRevert("Only the config owner or comptroller admin can call this function");
        distributor._updateOwner(hToken, address(emissionToken), address(100));
    ...
    function testConfigAssertValues() public {
        ...
        MultiRewardDistributorCommon.MarketConfig[] memory allConfigs = distributor.getAllMarketConfigs(hToken);

        assertEq(allConfigs.length, 1);
        assertEq(distributor.getCurrentOwner(hToken, address(emissionToken)), address(1));
    }
    ...
    function testUpdateEndTimeConfigOwnerFails() public {
        ...
        vm.prank(address(2));
        vm.expectRevert("Only the config owner or comptroller admin can call this function");
        distributor._updateEndTime(hToken, address(emissionToken), vm.getBlockTimestamp() + 366 days);
    }
}
