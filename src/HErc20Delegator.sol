...
/**
 * @title HErc20Delegator Contract
 * @notice RTokens which wrap an EIP-20 underlying and delegate to an implementation
 */
contract HErc20Delegator is HTokenInterface, HErc20Interface, HDelegatorInterface {
    /**
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData)
        ...
        override
    {
        require(msg.sender == admin, "HErc20Delegator::_setImplementation: Caller must be admin");
    ...
    {
        bytes memory data = delegateToImplementation(
            abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, hTokenCollateral)
        );
        return abi.decode(data, (uint256));
    }
    ...
    fallback() external payable {
        require(msg.value == 0, "HErc20Delegator:fallback: cannot send value to fallback");
