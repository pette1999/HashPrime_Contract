...
contract CompositeOracle is OwnableUpgradeable, ICompositeOracle {
    ...
    function setHTokenConfig(HToken hToken, address underlyingAddr, uint8 decimals) external onlyOwner {
        ...
        configs[hTokenAddr] = RTokenConfig({underlying: ERC20(underlyingAddr), hToken: hToken, decimals: decimals});

        emit RTokenConfigUpdate(hTokenAddr, underlyingAddr);
    }
    ...
    function getRTokenConfig(HToken hToken) public view returns (RTokenConfig memory) {
        RTokenConfig memory data = configs[address(hToken)];
        return data;
    }
