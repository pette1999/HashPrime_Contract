...
abstract contract HToken is HTokenInterface, Exponential, TokenErrorReporter {
    ...
    function transfer(address dst, uint256 amount) external override nonReentrant returns (bool) {
        if (dst == address(this) && msg.sender != admin) {
            return false;
        }
    ...
    /**
     * @notice Returns the current per-timestamp borrow interest rate for this hToken
    ...
    /**
     * @notice Returns the current per-timestamp supply interest rate for this hToken
    ...
    function accrueInterest() public virtual override returns (uint256) {
        ...
        (MathError mathErr, uint256 blockDelta) = subUInt(currentBlockTimestamp, accrualBlockTimestampPrior);
    ...
    function mintFresh(address minter, uint256 mintAmount) internal returns (uint256, uint256) {
         ...
         *  Note: The hToken must handle variations between ERC-20 and GLMR underlying.
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint256 repayAmount, HTokenInterface hTokenCollateral)
        internal
        nonReentrant
        returns (uint256, uint256)
    {
    ...
    function liquidateBorrowFresh(
        ...
        uint256 allowed = comptroller.liquidateBorrowAllowed(
            address(this), address(hTokenCollateral), liquidator, borrower, repayAmount
        );
        if (allowed != 0) {
        ...
        /* Verify hTokenCollateral market's block timestamp equals current block timestamp */
        if (hTokenCollateral.accrualBlockTimestamp() != getBlockTimestamp()) {
        ...
        (uint256 amountSeizeError, uint256 seizeTokens) =
            comptroller.liquidateCalculateSeizeTokens(address(this), address(hTokenCollateral), actualRepayAmount);
        ...
        /* We call the defense hook */
        // unused function
        // comptroller.liquidateBorrowVerify(address(this), address(hTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
